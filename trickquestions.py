...
# List of trick questions and their answers
QUESTIONS = [
 ...
 {'question': "How many legs does a dog have if you count its tail as a leg?",
  ...
  'accept': ['four', '4']},
 {'question': "How much more are pennies worth compared to pennies?",
  'answer': "One cent.",
  'accept': ['1', 'one']},
...
def display_question(question_number, score, question_text):
    print('Question:', question_number)
    print('Score:', score, '/ 3')
    print('QUESTION:', question_text)

def get_user_input():
    return input('  ANSWER: ').lower()

def is_correct(response, accepted_answers):
    return any(word in response for word in accepted_answers)

def display_result(text, answer):
    print(text, answer)
...
def play_game():
    ...
    for questionNumber, qa in enumerate(QUESTIONS, 1):  # Main program loop.
        display_question(questionNumber, score, qa['question'])
        ...
        if is_correct(response, qa['accept']):
            text = random.choice(CORRECT_TEXT)
            display_result(text, qa['answer'])
            score += 1
            if score == 3:
             wonTrick = True
             return wonTrick
            ...
            display_result(text, 'The answer is: ' + qa['answer'])
            chancesLeft -= 1
        ...
        input('Press Enter for the next question...').lower()
